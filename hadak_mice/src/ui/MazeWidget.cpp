...
void MazeWidget::setEditMode(bool enabled) {
  m_editMode = enabled;
  update();
}
...
void MazeWidget::drawMouse(QPainter *painter, const QRectF &bounds) {
  ...
  if (m_showSensors) {
    ...
    QPair<int, int> leftDelta = deltaFor(rotateLeft90(m_sim->mouse().heading()));
    QPointF left(cx + leftDelta.first * cellSize, cy - leftDelta.second * cellSize);
    painter->drawLine(QPointF(cx, cy), left);

    QPair<int, int> rightDelta = deltaFor(rotateRight90(m_sim->mouse().heading()));
    QPointF right(cx + rightDelta.first * cellSize, cy - rightDelta.second * cellSize);
...
void MazeWidget::mousePressEvent(QMouseEvent *event) {
  if (!m_editMode || !m_sim || !m_sim->maze()) {
