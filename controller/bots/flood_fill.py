...
def goal_cells(width, height):
    try:
        count = int(send("goalCount"))
    except ValueError:
        return center_cells(width, height)
    goals = set()
    for i in range(count):
        parts = send(f"goalCell {i}").split()
        if len(parts) != 2:
            continue
        try:
            x = int(parts[0])
            ...
            continue
        goals.add((x, y))
    return goals or center_cells(width, height)
